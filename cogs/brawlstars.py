...
class Brawlstars(commands.Cog):
    ...
    @commands.slash_command(description='Look up the counters of brawlers!!!', guild_ids=GUILD_IDS)  # ct command
    @option('brawler', str, description="Brawler to search up", autocomplete=basic_autocomplete(brawler_names))
    ...
    @commands.slash_command(description='Get detailed info on drafting on any Power League map', guild_ids=GUILD_IDS)  # map command
    @option('map', str, description="Map to search up", autocomplete=basic_autocomplete(maplist_names))
