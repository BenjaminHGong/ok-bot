...
class Economy(commands.Cog):
    ...
    @commands.slash_command(description='Check anyone\'s Ok Bot balance'
                            )  # balance command
    @option('member',
            discord.Member,
            description="Person to check for balance")
            ...
            if user_money > 5000:
                if a > 60:
                    ...
                    rob_embed = discord.Embed(
                        ...
                        description=
                        f"You have stolen {money} coins from <@{user.id}>",
                        color=discord.Color.green())
                ...
                await ctx.respond(f'<@{user.id}> dosent have enough money to get robbed')

        else:
            await ctx.respond('You don\'t have enough money to rob (wallet money > 5000)')

    @commands.slash_command(description='Deposit coins into the bank')  # deposit command
    ...
    @commands.slash_command(description='Withdraw coins from the bank'
                            )  # withdraw command
    ...
    async def update_bank(self,
                          user,
                          change=0,
                          mode='wallet'):  # update_bank function
        users = await self.get_bank_data()
    ...
    @commands.slash_command(description='Check what items you have'
                            )  # bag command
    async def bag(self, ctx):
        ...
        if bag == []:
            em = discord.Embed(title="Bag", color=discord.Color.blue(), description='You have nothing in your bag')
    ...
    @commands.slash_command(description='Buy something with your coins'
                            )  # buy command
    ...
    @commands.slash_command(description='Sell items in your bag'
                            )  # sell command
    ...
    async def sell_this(self,
                        user,
                        item_name,
                        amount,
                        price=None):  # sell_this function
        item_name = item_name.lower()
        name_ = None
        for item in mainshop:
            name = item["name"].lower()
    ...
    @commands.slash_command(description='Use something in your bag'
                            )  # use command
    ...
    @commands.slash_command(description='Check out the richest people')  # baltop command
    ...
    async def baltop(self, ctx, x=10):
        ...
        for id_ in leader_board:
            ...
            em.add_field(name=f"{index}. {member.name}",
                         value=f"{coins}",
                         inline=False)
            if index == x:
                break
    ...
    async def gamble(self, ctx, amount=500):
        ...
        bal = await self.update_bank(user)
        if amount < 500:
            return await ctx.respond(embed=discord.Embed(description="You need to bet at least 500 coins.", color=discord.Color.red()))

        if bal[0] < amount:
            return await ctx.respond(embed=discord.Embed(description="You don't have enough money in your wallet.", color=discord.Color.red()))
        ...
        if userroll > botroll:
            ...
            embed = discord.Embed(title='You Won!', description=f"Winnings: {winnings}", color=discord.Color.green())
            embed.add_field(name=f"{user} rolled", value=f'{userroll}', inline=True)
            embed.add_field(name=f"{self.bot.user} rolled", value=f'{botroll}', inline=True)
            ...
            embed = discord.Embed(title='You Lost...', description=f"Losses: {losses}", color=discord.Color.red())
            embed.add_field(name=f"{user} rolled", value=f'{userroll}', inline=True)
            embed.add_field(name=f"{self.bot.user} rolled", value=f'{botroll}', inline=True)
            embed.add_field(name=DESC, value=f'{bal[0]}', inline=False)
            return await ctx.respond(embed=embed)
