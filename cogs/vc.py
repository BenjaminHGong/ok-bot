...
class VC(commands.Cog):
    ...
    @vc.command(description='Tells the bot to join the voice channel', guild_ids=GUILD_IDS)
    ...
    @vc.command(description='Tells the bot to leave the voice channel', guild_ids=GUILD_IDS)
    ...
    @option('sound', str, description="Sound to play", autocomplete=basic_autocomplete(sound_names))
    async def soundboard(self, ctx, sound):
        vc = ctx.voice_client
        if not vc: # check if the bot is not in a voice channel
            await ctx.respond("The bot is not connected to a voice channel.")

        elif ctx.author.voice.channel.id != vc.channel.id: # check if the bot is not in the voice channel
            return await ctx.respond("You must be in the same voice channel as the bot.") # return an error message
        
        else:
            audio = discord.FFmpegPCMAudio(executable="C:/ffmpeg/bin/ffmpeg.exe", source=f"sounds/{sound}.mp3")
            if vc.is_playing():
                await ctx.respond('Please wait until the current sound has finished.', ephemeral=True)
            else:
                vc.play(audio)
